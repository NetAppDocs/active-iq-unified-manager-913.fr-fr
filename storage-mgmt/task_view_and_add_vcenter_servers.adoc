...
** Orange : « erreur » (indique toute erreur interne, telle qu'une exception, une défaillance dans la collecte des données de configuration ou un serveur vCenter inaccessible)
Vous pouvez cliquer sur l'icône d'affichage de la colonne (*Afficher/Masquer*) pour afficher le message d'état d'un serveur vCenter et résoudre le problème.
