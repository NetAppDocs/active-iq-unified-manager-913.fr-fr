...
Voir link:concept_cluster_components_and_why_they_can_be_in_contention.html["Les composants du cluster et les conflits"] pour obtenir une description des composants du cluster qui sont affichés ici.
 ** *Throughput* affiche à la fois le débit en IOPS et en Mo/s pour la charge de travail sur la période sélectionnée. Le graphique comporte quatre vues qui vous permettent de voir :
  *** *Débit total*
  *** * Débit de ventilation* (ventilé par lectures, écritures et autres processus)
  *** *Cloud Throughput* (Mo/s utilisés pour écrire des données dans le cloud et les lire ; pour les workloads qui hiérarchise la capacité vers le cloud)
  *** *IOPS avec prévisions* (prévision des valeurs de débit d'IOPS supérieures et inférieures sur la période)
Ce tableau affiche également les paramètres de seuils de débit maximal et minimal de qualité de service (QoS), s'ils sont configurés, pour vous permettre de voir où le système peut limiter le débit intentionnellement dans les règles de QoS.
...
** *Cloud Tier View* affiche la capacité utilisée dans le Tier de performance local et la quantité utilisée dans le Tier cloud.
